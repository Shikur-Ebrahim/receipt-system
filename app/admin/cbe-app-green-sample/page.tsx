...
export default function CbeAppGreenSamplePage() {
  ...
  const [textColor, setTextColor] = useState("rgb(5, 0, 0)");
  // Default OFF because your uploaded template already has the "Message" label and blank area.
  const [hideOriginalMessageArea, setHideOriginalMessageArea] = useState(false);
  // Start just below the "Message" label area.
  const [messageBox, setMessageBox] = useState<MessageBox>({ x: 56, y: 210, w: 877, h: 300 });
    ...
    const load = async () => {
      ...
      try {
        ...
        if (items.length > 0) {
          ...
          setTemplateUrl(null);
          setTemplateError("Upload a CBE App Green template first (Admin → CBE App Green Template).");
          return;
        }
      } catch (e) {
        console.error(e);
        setTemplateError("Failed to load the stored CBE App Green template.");
      } finally {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      a.href = image;
      a.download = `CBE-App-Green-Receipt-${Date.now()}.png`;
      a.click();
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="mb-10">
        <h1 className="text-3xl font-black text-gray-800 tracking-tight">CBE App Green Message Editor</h1>
        <p className="text-gray-400 font-bold text-sm mt-2">
          Upload the first sample screenshot, then edit only the message section to produce the filled second-style output.
        </p>
      </div>
            ...
            <div className={`flex flex-col items-center ${activeTab === "preview" ? "" : "hidden"} sm:flex`}>
              <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm p-6 md:p-8 w-full flex flex-col items-center">
                ...
                <div className="w-full overflow-x-auto">
                  <div
                    className="relative bg-white rounded-[2rem] border border-gray-100 overflow-visible min-w-[538px]"
                    ref={receiptWrapRef}
                    style={{ width: previewW, height: previewH }}
                    aria-label="CBE app green receipt preview"
                  >
                    <img
                      src={templateUrl}
