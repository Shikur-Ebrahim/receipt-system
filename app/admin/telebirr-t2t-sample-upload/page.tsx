...
const DEFAULT_COLLECTION = "telebirr_t2t_sample";
const TEMPLATE_DOC_ID = "template";
const TEMPLATE_SUBCOLLECTION = "templates";

export default function TelebirrT2TSampleUploadPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        console.error("Failed to load telebirr T2T template:", e);
      }
    };
    load();
  }, []);
  ...
  const handleUploadAndSave = async () => {
    if (!selectedFiles.length) {
      alert("Select at least one Telebirr → Telebirr sample template image first.");
      return;
    }
    ...
    try {
      ...
      for (const file of selectedFiles) {
        ...
        // Keep updating the "latest template" doc (default template).
        await setDoc(
          doc(db, DEFAULT_COLLECTION, TEMPLATE_DOC_ID),
      ...
      setSaving(false);
      await loadTemplates();
      alert(`Telebirr → Telebirr template(s) saved successfully: ${selectedFiles.length}`);
    } catch (e) {
      console.error(e);
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="mb-10">
        <h1 className="text-3xl font-black text-gray-800 tracking-tight">Telebirr → Telebirr Template Upload</h1>
        <p className="text-gray-400 font-bold text-sm mt-2">
          Upload the Telebirr→Telebirr sample template screenshot(s). Stored in Cloudinary; URLs saved to Firestore.
        </p>
      </div>
        ...
        <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm p-6 md:p-8">
          ...
          <label className="block w-full">
            <div className="border-4 border-dashed border-gray-200 rounded-[2rem] p-6 md:p-8 text-center cursor-pointer hover:border-[#8cc63f]/50 transition-colors">
              ...
              <div className="text-xs text-gray-400 font-bold mt-2">PNG/JPG. Use the Telebirr→Telebirr layout.</div>
              <input
                type="file"
                accept="image/*"
                multiple
          ...
          {templateUrl ? (
            ...
            <div className="text-center p-10">
              ...
              <p className="text-gray-500 font-bold text-sm mt-2">
                Upload a template to enable the Telebirr→Telebirr editor page.
              </p>
            </div>
          )}
                ...
                {templates.map((t) => (
                  <div key={t.id} className="bg-gray-50 border border-gray-100 rounded-[1.25rem] p-3">
                    ...
                    <div className="mt-2">
                      <img
                        src={t.templateUrl}
                        alt="Telebirr T2T uploaded template"
