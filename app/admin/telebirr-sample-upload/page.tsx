...
const DEFAULT_COLLECTION = "telebirr_sample";
const TEMPLATE_DOC_ID = "template";
const TEMPLATE_SUBCOLLECTION = "templates";

export default function TelebirrSampleUploadPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        console.error("Failed to load telebirr template:", e);
      }
    };
    load();
  }, []);
  ...
  const handleDeleteTemplate = async (id: string) => {
    ...
    try {
      ...
      // If the "latest" template doc was pointing to the deleted image, update it to another one (or clear).
  ...
  const cloudinaryUpload = async (file: File) => {
    ...
    formData.append(
      "upload_preset",
      process.env.NEXT_PUBLIC_CLOUDINARY_UPLOAD_PRESET || "pioneerbusiness"
    );
  ...
  const handleUploadAndSave = async () => {
    if (!selectedFiles.length) {
      alert("Select at least one Telebirr sample image first.");
      return;
    }
    ...
    try {
      ...
      for (const file of selectedFiles) {
        ...
        // Keep updating the "latest template" for backward compatibility / default behavior.
        await setDoc(
          doc(db, DEFAULT_COLLECTION, TEMPLATE_DOC_ID),
          {
            kind: "template",
        ...
        // Store every upload as a separate template.
      ...
      setSaving(false);
      await loadTemplates();
      alert(`Telebirr template(s) saved successfully: ${selectedFiles.length}`);
    } catch (e) {
      console.error(e);
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="mb-10">
        <h1 className="text-3xl font-black text-gray-800 tracking-tight">Telebirr Sample Upload</h1>
        <p className="text-gray-400 font-bold text-sm mt-2">
          Upload the Telebirr sample template screenshot. This is stored in Cloudinary and the URL is saved to Firestore.
        </p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
        <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm p-6 md:p-8">
          <h2 className="text-xl font-black text-gray-800 tracking-tight mb-4">Upload Template Image</h2>
          ...
          <label className="block w-full">
            <div className="border-4 border-dashed border-gray-200 rounded-[2rem] p-6 md:p-8 text-center cursor-pointer hover:border-[#8cc63f]/50 transition-colors">
              <div className="text-gray-500 font-black text-sm uppercase tracking-widest">Select Image</div>
              <div className="text-xs text-gray-400 font-bold mt-2">
                PNG/JPG. Use the same Telebirr sample layout (the one with hidden fields).
              </div>
              <input
                type="file"
                accept="image/*"
          ...
          <div className="mt-6 space-y-4">
            ...
            {selectedFiles.length > 0 && (
              <p className="text-xs text-gray-500 font-bold break-all">
          ...
          {templateUrl ? (
            <div>
              <div className="bg-gray-50 border border-gray-100 rounded-[2rem] p-4">
                <img
                  ...
                  className="w-full max-w-[360px] mx-auto rounded-2xl border border-gray-100 object-contain"
                />
              </div>
            ...
            <div className="text-center p-10">
              ...
              <p className="text-gray-500 font-bold text-sm mt-2">
                Upload the sample template on the left to enable the editor page.
              </p>
            </div>
          )}
              ...
              <div className="max-h-[260px] overflow-y-auto space-y-3 pr-2">
                {templates.map((t) => (
                  <div key={t.id} className="bg-gray-50 border border-gray-100 rounded-[1.25rem] p-3">
                    <p className="text-[11px] text-gray-500 font-bold break-all">
                      {t.templateName ? t.templateName : t.id}
                    </p>
                    <div className="mt-2">
                      <img
                        src={t.templateUrl}
                        alt="Telebirr uploaded template"
                        className="w-full max-w-[320px] rounded-xl border border-gray-100 object-contain"
                      />
                    </div>
                    <div className="mt-3">
                      <button
