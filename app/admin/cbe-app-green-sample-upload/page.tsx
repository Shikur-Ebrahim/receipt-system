...
const DEFAULT_COLLECTION = "cbe_app_green_sample";
const TEMPLATE_DOC_ID = "template";
const TEMPLATE_SUBCOLLECTION = "templates";

export default function CbeAppGreenSampleUploadPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        console.error("Failed to load CBE app green template:", e);
      }
    };
    load();
  }, []);
  ...
  const handleUploadAndSave = async () => {
    if (!selectedFiles.length) {
      alert("Select at least one CBE app green screenshot first.");
      return;
    }
    ...
    try {
      ...
      setSaving(false);
      await loadTemplates();
      alert(`CBE app green template(s) saved: ${selectedFiles.length}`);
    } catch (e) {
      console.error(e);
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="mb-10">
        <h1 className="text-3xl font-black text-gray-800 tracking-tight">CBE App Green Template Upload</h1>
        <p className="text-gray-400 font-bold text-sm mt-2">
          Upload the first sample image (green receipt screenshot). This image becomes your editable base template.
        </p>
      </div>
        ...
        <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm p-6 md:p-8">
          ...
          <label className="block w-full">
            <div className="border-4 border-dashed border-gray-200 rounded-[2rem] p-6 md:p-8 text-center cursor-pointer hover:border-[#8cc63f]/50 transition-colors">
              ...
              <div className="text-xs text-gray-400 font-bold mt-2">PNG/JPG. Upload the blank/first screenshot.</div>
              <input
                type="file"
                accept="image/*"
                multiple
                ...
                {templates.map((t) => (
                  <div key={t.id} className="bg-gray-50 border border-gray-100 rounded-[1.25rem] p-3">
                    ...
                    <div className="mt-2">
                      <img
                        src={t.templateUrl}
                        alt="CBE app green uploaded template"
