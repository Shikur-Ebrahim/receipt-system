...
const DEFAULT_COLLECTION = "business_license_sample";
const TEMPLATE_DOC_ID = "template";
const TEMPLATE_SUBCOLLECTION = "templates";

export default function BusinessLicenseSampleUploadPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        console.error("Failed to load business license template:", e);
      }
    };
    load();
  }, []);
  ...
  const handleUploadAndSave = async () => {
    if (!selectedFiles.length) {
      alert("Select at least one business license scan image first.");
      return;
    }
    ...
    try {
      ...
      setSaving(false);
      await loadTemplates();
      alert(`Business license template(s) saved: ${count}`);
    } catch (e) {
      console.error(e);
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="mb-10">
        <h1 className="text-3xl font-black text-gray-800 tracking-tight">Business License Template Upload</h1>
        <p className="text-gray-400 font-bold text-sm mt-2">
          Upload the original blank or sample business license scan. This image becomes the base for the bilingual editor.
        </p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
        <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm p-6 md:p-8">
          <h2 className="text-xl font-black text-gray-800 tracking-tight mb-4">Upload Image</h2>

          <label className="block w-full">
            <div className="border-4 border-dashed border-gray-200 rounded-4xl p-6 md:p-8 text-center cursor-pointer hover:border-[#8cc63f]/50 transition-colors">
              <div className="text-gray-500 font-black text-sm uppercase tracking-widest">Select Image(s)</div>
              <div className="text-xs text-gray-400 font-bold mt-2">PNG/JPG. Use a clear scan of the license form.</div>
              <input
                type="file"
                accept="image/*"
                multiple
          ...
          <div className="mt-6 space-y-4">
            <button
              ...
              className="w-full bg-[#8cc63f] hover:bg-[#7ab32f] text-white font-black py-4 rounded-3xl transition-all active:scale-[0.98] disabled:opacity-60 shadow-lg shadow-[#8cc63f]/20 text-sm uppercase tracking-widest"
            >
              {uploading ? "Uploading..." : saving ? "Saving to Firestore..." : "Upload & Save Template"}
            </button>
        ...
        <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm p-6 md:p-8">
          ...
          {templateUrl ? (
            <div>
              <div className="bg-gray-50 border border-gray-100 rounded-4xl p-4">
                <img
                  src={templateUrl}
                  alt="Business license template"
                  className="w-full max-w-[320px] mx-auto rounded-2xl border border-gray-100 object-contain"
                />
              </div>
            ...
            <div className="text-center p-10">
              ...
              <p className="text-gray-500 font-bold text-sm mt-2">Upload a scan to enable the editor page.</p>
            </div>
          )}
                ...
                {templates.map((t) => (
                  <div key={t.id} className="bg-gray-50 border border-gray-100 rounded-[1.25rem] p-3">
                    ...
                    <div className="mt-2">
                      <img
                        src={t.templateUrl}
                        alt="Uploaded business license template"
                        className="w-full max-w-[280px] rounded-xl border border-gray-100 object-contain"
                      />
                    </div>
                    <div className="mt-3">
                      <button
